...
/////////////////////////////////////////////////////////////////////////////////////
cScene Scene;
//display resolution when game scene loads
unsigned int initWidth = 1366;
unsigned int initHeight = 960;
...
void resize(GLFWwindow* window, int width, int height)
{
	...
	//FPS COUNTER ATTEMPT TO IMPLEMENT, to_string DOES NOT WORK
	/*timeDiff = crntTime - prevTime;
	if (timeDiff >= 1.0 / 30.0) {
		std::string FPS = std::to_string((1.0 / timeDiff) * counter);
		std::string ms = std::to_string((timeDiff / counter) * 1000);
		std::string newTitle = "CIS6015_PRAC1 - " + FPS + "FPS / " + ms + "ms";
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods) 
{
	...
	switch (key)
	{
		...
		//WASD key inputs can be replaced for player movement if classes are implemented
		...
		case GLFW_KEY_SPACE: {//input for camera swap
...
int main() 
{
	...
	GLFWwindow* window = glfwCreateWindow(initWidth, initHeight, "CIS6015_PRAC1 Graphics Template OpenGL", NULL, NULL);
	if (window == NULL)
	{
